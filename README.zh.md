...
| 图标 | 功能 |
|---|---|
| ⚡ | **实测更快更省**：同一任务对比裸 DSH，耗时 -33%、估算 token -44%（小样本，未大规模测试） |
...
| 指标 | 装套件 | 裸 DSH | 差距 |
|---|---|---|---|
| **总耗时** | 8.6 分钟 | 12.8 分钟 | **省 33% 时间** |
| LLM 调用次数 | 51 次 | 61 次 | 少 20% |
| 工具调用次数 | 59 次 | 67 次 | 少 14% |
| **估算总 token** | ~41,000 | ~73,000 | **省 44% 流量** |
| 反复推理次数 | 401 | 1,163 | 少 65% |

**过程体验差距**：装套件后，遇到问题不硬磕（会自动换思路）、把好用的命令沉淀成脚本复用、关键节点会先问你再动手；裸 DSH 会在同一个问题上反复试 13 次、不沉淀方法、也不征求你意见。
...
DSH 功能齐全，但**不够聪明**：系统提示词是空的、失败了会死磕、技能装了不用、规划工具爱用不用。本套件用两层补齐：
...
| `dsh-memory` | 记忆重复膨胀、乱用强制写入 | 写入前查重（相似度超 0.62 拒绝）+ 60 秒窗口去重 |
...
| `discipline-hub` | 插件各管各的、提醒刷屏 | 统一记录失败 + 提醒限流（每步最多 2 条） |
...
| `dsh-error-triage` | 错误速查表：什么错对应什么命令（覆盖 8 类错误） |
...
cd tests
node test-anti-stuck.mjs        # 41 项
node test-cordis-discipline.mjs # 44 项
node test-discipline-hub.mjs    # 35 项
node test-dsh-discipline.mjs    # 24 项
node test-dsh-memory.mjs        # 48 项
node test-env-check.mjs         # 61 项
node test-env-check-tool.mjs    # 15 项
node test-env-triage.mjs        # 22 项
node test-fast-locate.mjs       # 32 项
node test-plan-discipline.mjs   # 29 项
node test-skill-evolver.mjs     # 33 项
node test-skill-loader.mjs      # 42 项
# 合计 426 项
```

**覆盖率**（v1.2 重构后测量）：全局语句 100% / 分支 96.74% / 函数 98.91% / 行 100%；7 个源文件全维度 100%。未覆盖项均为已确认的死分支或无法安全测试的项（详见 `docs/TESTING.md` 第二节）。

另有回放验证工具 `tests/replay-discipline.mjs`：把真实 DSH 会话日志喂给插件统计干预覆盖率，量化"改进前 vs 改进后"（实测干预从 4 次提到 11 次，+175%）。

---

## 设计理念

- **从真实日志出发**：每个插件都来自真实长任务的失败分析（死磕几次、该用没用、失败原因），不是拍脑袋
- **无提示自测**：验收测试的提问里完全不提插件——只有 AI 在没人提醒的情况下主动用，才证明真的学会了
- **可观测**：环境体检（插件语法/配置一致性/技能合规）+ 部署闭环（同步→验证→重启→复验），杜绝"改了没生效"
...
*详细文档：`docs/ARCHITECTURE.md`（架构与协作框架）、`docs/TESTING.md`（验证方法论）*
